...
def make_normalize() -> onnx.FunctionProto:
    eps: onnx.NodeProto = onnx.helper.make_node("Constant", [], ["eps"],
                                                value_float=1e-12)
    axes: onnx.NodeProto = onnx.helper.make_node("Constant", [], ["axes"],
                                                 value_ints=[-1])
    x: onnx.NodeProto = onnx.helper.make_node("Cast", ["x_"], ["x"],
    ...
    normalize: list[onnx.NodeProto] = [
        ...
        onnx.helper.make_node("CastLike", ["normalize_", "x_"], ["normalize"])
    ]

    return onnx.helper.make_function(__domain, "normalize", ["x_"],
                                     ["normalize"], [eps, axes, x] + normalize,
                                     __opset_imports)


def make_linear() -> onnx.FunctionProto:
    weight_t: onnx.NodeProto = onnx.helper.make_node("Transpose", ["weight"],
                                                     ["weight.T"])
    x_weight_t: onnx.NodeProto = onnx.helper.make_node("MatMul",
                                                       ["x", "weight.T"],
                                                       ["x@weight.T"])
    return onnx.helper.make_function(__domain, "linear", ["x", "weight"],
                                     ["x@weight.T"], [weight_t, x_weight_t],
                                     __opset_imports)
...
def make_time_shift() -> onnx.FunctionProto:
    constants: list[onnx.NodeProto] = [
        ...
        onnx.helper.make_node("Constant", [], ["max"],
                              value_ints=[np.iinfo(np.int64).max]),
    ...
    x_shift: list[onnx.NodeProto] = [
        onnx.helper.make_node("Slice", ["x", "0", "x_end", "T_axes"],
                              ["x_shift_"]),
        onnx.helper.make_node("Concat", ["x_last", "x_shift_"], ["x_shift"],
                              axis=1)
    ]
    x_next: onnx.NodeProto = onnx.helper.make_node(
        "Slice", ["x", "x_end", "max", "T_axes"], ["x_next"])

    return onnx.helper.make_function(__domain, "time_shift", ["x", "x_last"],
                                     ["x_shift", "x_next"],
                                     constants + x_shift + [x_next],
                                     __opset_imports)
...
def make_lerp() -> onnx.FunctionProto:
    ...
    return onnx.helper.make_function(__domain, "lerp", ["a", "b", "x"],
                                     ["lerp"], lerp, __opset_imports)
...
def make_loramlp() -> list[onnx.FunctionProto]:
    ...
    identity_xA: onnx.NodeProto = onnx.helper.make_node(
        "Identity", ["xA"], ["f_xA"])
    sigmoid_xA: onnx.NodeProto = onnx.helper.make_node("Sigmoid", ["xA"],
                                                       ["f_xA"])
    tanh_xA: onnx.NodeProto = onnx.helper.make_node("Tanh", ["xA"], ["f_xA"])
    f_xAB: onnx.NodeProto = onnx.helper.make_node("MatMul", ["f_xA", "B"],
                                                  ["f_xAB"])

    f_xAB_identity: onnx.NodeProto = onnx.helper.make_node(
        "Identity", ["f_xAB"], ["loramlp"])
    f_xAB_bias: onnx.NodeProto = onnx.helper.make_node("Add",
                                                       ["f_xAB", "bias"],
                                                       ["loramlp"])

    return [
        onnx.helper.make_function(__domain, "loramlp_identity_bias",
                                  ["x", "A", "B", "bias"], ["loramlp"],
                                  [xA, identity_xA, f_xAB, f_xAB_bias],
                                  __opset_imports),
        onnx.helper.make_function(__domain, "loramlp_sigmoid_bias",
                                  ["x", "A", "B", "bias"], ["loramlp"],
                                  [xA, sigmoid_xA, f_xAB, f_xAB_bias],
                                  __opset_imports),
        onnx.helper.make_function(__domain, "loramlp_tanh_bias",
                                  ["x", "A", "B", "bias"], ["loramlp"],
                                  [xA, tanh_xA, f_xAB, f_xAB_bias],
                                  __opset_imports),
        onnx.helper.make_function(__domain, "loramlp_identity",
                                  ["x", "A", "B"], ["loramlp"],
                                  [xA, identity_xA, f_xAB, f_xAB_identity],
                                  __opset_imports),
        onnx.helper.make_function(__domain, "loramlp_sigmoid", ["x", "A", "B"],
                                  ["loramlp"],
                                  [xA, sigmoid_xA, f_xAB, f_xAB_identity],
                                  __opset_imports),
        onnx.helper.make_function(__domain, "loramlp_tanh", ["x", "A", "B"],
                                  ["loramlp"],
                                  [xA, tanh_xA, f_xAB, f_xAB_identity],
                                  __opset_imports),
    ]
...
def make_wkv7(dtype: int) -> onnx.FunctionProto:
    ...
    diag_w: onnx.NodeProto = onnx.helper.make_node("Unsqueeze",
                                                   ["w", "diag_w_unsqueeze"],
                                                   ["diag_w"])
    wkv_w: onnx.NodeProto = onnx.helper.make_node("Mul",
                                                  ["wkv_state", "diag_w"],
                                                  ["wkv_w"])
    wkv_ab: onnx.NodeProto = onnx.helper.make_node("MatMul",
                                                   ["wkv_state", "ab"],
                                                   ["wkv_ab"])
    ...
    wkv_out: onnx.NodeProto = onnx.helper.make_node("Identity", ["wkv_next"],
                                                    ["wkv_out"])
    ...
    w_float: onnx.NodeProto = onnx.helper.make_node("Cast", ["w"], ["w_float"],
                                                    to=dtype)
    ab_float: onnx.NodeProto = onnx.helper.make_node("Cast", ["ab"],
                                                     ["ab_float"],
                                                     to=dtype)
    vk_float: onnx.NodeProto = onnx.helper.make_node("Cast", ["vk"],
                                                     ["vk_float"],
                                                     to=dtype)
    ...
    rwkv: [onnx.NodeProto] = [
        ...
        onnx.helper.make_node("MatMul", ["wkv_out_casted", "r_unsqueeze"],
                              ["rwkv"])
    ]

    return onnx.helper.make_function(
        __domain, "wkv7", ["wkv_state", "r", "w", "k", "v", "a", "b"],
        ["wkv_state_next", "rwkv"], constants + broadcast + ab + vk +
        [w_float, ab_float, vk_float, wkv_loop, wkv_out_casted] + rwkv,
        __opset_imports)


def make_time_mix(dim: int, head_size: int,
                  dim_att: int) -> list[onnx.FunctionProto]:
    ...
    constants: list[onnx.NodeProto] = [
        ...
        onnx.helper.make_node("Constant", [], ["N"], value_ints=[head_size]),
        ...
        onnx.helper.make_node("Constant", [], ["(-exp(-0.5))"],
                              value_float=-np.exp(-0.5)),
        onnx.helper.make_node("CastLike", ["(-exp(-0.5))", "x"],
                              ["-exp(-0.5)"]),
        onnx.helper.make_node("Constant", [], ["[0]"], value_ints=[0]),
        onnx.helper.make_node("Constant", [], ["[1]"], value_ints=[1]),
    ...
    token_shift: list[onnx.NodeProto] = [
        onnx.helper.make_node("time_shift", ["x", "x_last"],
                              ["x_shift", "x_next"],
                              domain=__domain),
        onnx.helper.make_node("lerp", ["x", "x_shift", "x_r"], ["xr"],
                              domain=__domain),
        onnx.helper.make_node("lerp", ["x", "x_shift", "x_w"], ["xw"],
                              domain=__domain),
        onnx.helper.make_node("lerp", ["x", "x_shift", "x_k"], ["xk"],
                              domain=__domain),
        onnx.helper.make_node("lerp", ["x", "x_shift", "x_v"], ["xv"],
                              domain=__domain),
        onnx.helper.make_node("lerp", ["x", "x_shift", "x_a"], ["xa"],
                              domain=__domain),
        onnx.helper.make_node("lerp", ["x", "x_shift", "x_g"], ["xg"],
                              domain=__domain)
    ]

    weight_prepare: list[onnx.NodeProto] = [
        onnx.helper.make_node("linear", ["xr", "receptance.weight"], ["r"],
                              domain=__domain),
        onnx.helper.make_node("loramlp_tanh_bias", ["xw", "w1", "w2", "w0"],
                              ["w_"],
                              domain=__domain),
        onnx.helper.make_node("linear", ["xk", "key.weight"], ["k_"],
                              domain=__domain),
        onnx.helper.make_node("linear", ["xv", "value.weight"], ["v_"],
                              domain=__domain),
        onnx.helper.make_node("loramlp_identity_bias",
                              ["xa", "a1", "a2", "a0"], ["a_"],
                              domain=__domain),
        onnx.helper.make_node("Sigmoid", ["a_"], ["a"]),
        onnx.helper.make_node("loramlp_sigmoid", ["xg", "g1", "g2"], ["g"],
                              domain=__domain),
        onnx.helper.make_node("Mul", ["k_", "k_k"], ["k_*k_k"]),
        onnx.helper.make_node("Reshape", ["k_*k_k", "BTHN"], ["k_*k_k:head"]),
        onnx.helper.make_node("normalize", ["k_*k_k:head"], ["kk:head"],
                              domain=__domain),
        onnx.helper.make_node("lerp", ["1.0", "a", "k_a"],
                              ["lerp(1, a, alpha)"],
                              domain=__domain),
        onnx.helper.make_node("Mul", ["k_", "lerp(1, a, alpha)"], ["k"]),
    ...
    v_l: list[onnx.NodeProto] = [
        ...
        onnx.helper.make_node("lerp", ["v_", "v_first", "vg"], ["v"],
                              domain=__domain)
    ]
    ...
    wkv: onnx.NodeProto = onnx.helper.make_node("wkv7", [
        "wkv_state", "r:head", "w:head", "k:head", "v:head", "kk:head",
        "b:head"
    ], ["wkv_state_next", "wkv_out"],
                                                domain=__domain)

    readout: list[onnx.NodeProto] = [
        onnx.helper.make_node("Reshape", ["wkv_out", "GC"], ["wkv_out:group"]),
        onnx.helper.make_node("GroupNormalization",
                              ["wkv_out:group", "ln_x.weight", "ln_x.bias"],
                              ["wkv_out_gn:group"],
                              epsilon=64e-5,
                              num_groups=n_head),
        onnx.helper.make_node("Reshape", ["wkv_out_gn:group", "BTC"],
                              ["wkv_out_gn"]),
        ...
        onnx.helper.make_node("Reshape", ["r*k*r_k", "BTHN"],
                              ["r*k*r_k:head"]),
        onnx.helper.make_node("ReduceSum", ["r*k*r_k:head", "[-1]"], ["rk"]),
        onnx.helper.make_node("Mul", ["rk", "v:head"], ["rkv:head"]),
        onnx.helper.make_node("Reshape", ["rkv:head", "BTC"], ["rkv"]),
        onnx.helper.make_node("Add", ["wkv_out_gn", "rkv"],
                              ["wkv_out_gn+rkv"]),
        onnx.helper.make_node("Mul", ["wkv_out_gn+rkv", "g"],
                              ["(wkv_out_gn+rkv)*g"]),
        onnx.helper.make_node("linear",
                              ["(wkv_out_gn+rkv)*g", "output.weight"],
                              ["output"],
                              domain=__domain)
    ]
    ...
    return [
        onnx.helper.make_function(
            __domain, "time_mix0", [
                "x", "x_last", "wkv_state", "x_r", "x_w", "x_k", "x_v", "x_a",
                "x_g", "w1", "w2", "w0", "a1", "a2", "a0", "g1", "g2", "k_k",
                "k_a", "r_k", "receptance.weight", "key.weight",
                "value.weight", "output.weight", "ln_x.weight", "ln_x.bias"
            ], ["output", "v", "x_next", "wkv_state_next"],
            constants + token_shift + weight_prepare + [v_l0] + head_prepare +
            [wkv] + readout, __opset_imports),
        onnx.helper.make_function(
            __domain, "time_mix", [
                "x", "v_first", "x_last", "wkv_state", "x_r", "x_w", "x_k",
                "x_v", "x_a", "x_g", "w1", "w2", "w0", "a1", "a2", "a0", "v1",
                "v2", "v0", "g1", "g2", "k_k", "k_a", "r_k",
                "receptance.weight", "key.weight", "value.weight",
                "output.weight", "ln_x.weight", "ln_x.bias"
            ], ["output", "x_next", "wkv_state_next"],
            constants + token_shift + weight_prepare + v_l + head_prepare +
            [wkv] + readout, __opset_imports)
    ]
...
def make_channel_mix(C: int) -> onnx.FunctionProto:
    ...
    deepemb_shape: list[onnx.NodeProto] = [
        ...
        onnx.helper.make_node("Concat", ["B", "T", "[1]", "[32]"], ["BT1x32"],
                              axis=0),
        onnx.helper.make_node("Concat", ["B", "T", "[32]", "[32]"],
                              ["BT32x32"],
                              axis=0)
    ]

    time_shift: onnx.NodeProto = onnx.helper.make_node(
        "time_shift", ["x", "x_last"], ["x_shift", "x_last_next"],
        domain=__domain)
    lerp: onnx.NodeProto = onnx.helper.make_node("lerp",
                                                 ["x", "x_shift", "x_k"],
                                                 ["lerp"],
                                                 domain=__domain)
    ...
    k: onnx.NodeProto = onnx.helper.make_node("MatMul",
                                              ["lerp", "key_weight_t"], ["k"])
    relu: onnx.NodeProto = onnx.helper.make_node("Relu", ["k"], ["relu"])
    relu2: onnx.NodeProto = onnx.helper.make_node("Mul", ["relu", "relu"],
                                                  ["relu2"])
    x_s1_: onnx.NodeProto = onnx.helper.make_node("MatMul", ["x", "s1"],
                                                  ["x_s1_"])
    x_s1: onnx.NodeProto = onnx.helper.make_node("Reshape",
                                                 ["x_s1_", "BT1x32"], ["x_s1"])
    semb: onnx.NodeProto = onnx.helper.make_node("Reshape",
                                                 ["semb_", "BT32x32"],
                                                 ["semb"])
    x_s1_semb_: onnx.NodeProto = onnx.helper.make_node("MatMul",
                                                       ["x_s1", "semb"],
                                                       ["x_s1_semb_"])
    x_s1_semb: onnx.NodeProto = onnx.helper.make_node("Reshape",
                                                      ["x_s1_semb_", "BT32"],
                                                      ["x_s1_semb"])
    ...
    # For normal RWKV-7
    _deepemb: onnx.NodeProto = onnx.helper.make_node("Identity", ["relu2"],
                                                     ["deepemb"])

    v: onnx.NodeProto = onnx.helper.make_node("MatMul",
                                              ["deepemb", "value_weight_t"],
                                              ["v"])
    return [
        onnx.helper.make_function(
            ...
            ["v", "x_last_next"], [
                time_shift, lerp, key_weight_t, value_weight_t, k, relu, relu2,
                _deepemb, v
            ], __opset_imports),
        onnx.helper.make_function(
            __domain, "channel_mix_a", [
                "x", "x_last", "x_k", "key.weight", "value.weight", "s1",
                "semb_", "s2", "s0"
            ], ["v", "x_last_next"], deepemb_constants + deepemb_shape + [
                time_shift, lerp, key_weight_t, value_weight_t, k, relu, relu2,
                x_s1_, x_s1, semb, x_s1_semb_, x_s1_semb, x_s1_semb_s2,
                x_s1_semb_s2_s0, deepemb, v
            ], __opset_imports)
    ]
...
def make_block() -> list[onnx.FunctionProto]:
    ln0: onnx.NodeProto = onnx.helper.make_node(
        "LayerNormalization", ["emb", "ln0.weight", "ln0.bias"],
        ["emb_ln0", "emb_ln0_mean", "emb_ln0_invstdev"])
    ln0_: onnx.NodeProto = onnx.helper.make_node("Identity", ["emb"],
                                                 ["emb_ln0"])

    ln1: onnx.NodeProto = onnx.helper.make_node(
        "LayerNormalization", ["emb_ln0", "ln1.weight", "ln1.bias"],
        ["emb_ln1", "emb_ln1_mean", "emb_ln1_invstdev"])
    tmix0: onnx.NodeProto = onnx.helper.make_node("time_mix0", [
        "emb_ln1", "x_tmix_last", "wkv_state", "att.x_r", "att.x_w", "att.x_k",
        "att.x_v", "att.x_a", "att.x_g", "att.w1", "att.w2", "att.w0",
        "att.a1", "att.a2", "att.a0", "att.g1", "att.g2", "att.k_k", "att.k_a",
        "att.r_k", "att.receptance.weight", "att.key.weight",
        "att.value.weight", "att.output.weight", "att.ln_x.weight",
        "att.ln_x.bias"
    ], ["emb_tmix", "v_first", "x_tmix_next", "wkv_next"],
                                                  domain=__domain)
    tmix: onnx.NodeProto = onnx.helper.make_node("time_mix", [
        "emb_ln1", "v_first", "x_tmix_last", "wkv_state", "att.x_r", "att.x_w",
        "att.x_k", "att.x_v", "att.x_a", "att.x_g", "att.w1", "att.w2",
        "att.w0", "att.a1", "att.a2", "att.a0", "att.v1", "att.v2", "att.v0",
    ...
    ], ["emb_tmix", "x_tmix_next", "wkv_next"],
                                                 domain=__domain)
    tmix_x: onnx.NodeProto = onnx.helper.make_node("Add",
                                                   ["emb_ln0", "emb_tmix"],
                                                   ["emb_tmix_x"])
    ln2: onnx.NodeProto = onnx.helper.make_node(
        "LayerNormalization", ["emb_tmix_x", "ln2.weight", "ln2.bias"],
        ["emb_tmix_x_ln2", "emb_tmix_x_ln2_mean", "emb_tmix_x_ln2_invstdev"])
    cmix: onnx.NodeProto = onnx.helper.make_node("channel_mix", [
        "emb_tmix_x_ln2", "x_cmix_last", "ffn.x_k", "ffn.key.weight",
        "ffn.value.weight"
    ], ["emb_cmix", "x_cmix_next"],
                                                 domain=__domain)
    cmix_a: onnx.NodeProto = onnx.helper.make_node("channel_mix_a", [
        "emb_tmix_x_ln2", "x_cmix_last", "ffn.x_k", "ffn.key.weight",
        "ffn.value.weight", "ffn.s1", "semb", "ffn.s2", "ffn.s0"
    ], ["emb_cmix", "x_cmix_next"],
                                                   domain=__domain)
    cmix_x: onnx.NodeProto = onnx.helper.make_node("Add",
                                                   ["emb_tmix_x", "emb_cmix"],
                                                   ["emb_cmix_tmix_x"])

    return [
        onnx.helper.make_function(__domain, "block0", [
            "emb", "x_tmix_last", "wkv_state", "x_cmix_last", "ln0.weight",
            "ln0.bias", "ln1.weight", "ln1.bias", "ln2.weight", "ln2.bias",
            "att.x_r", "att.x_w", "att.x_k", "att.x_v", "att.x_a", "att.x_g",
            "att.w1", "att.w2", "att.w0", "att.a1", "att.a2", "att.a0",
            "att.g1", "att.g2", "att.k_k", "att.k_a", "att.r_k",
            "att.receptance.weight", "att.key.weight", "att.value.weight",
            "att.output.weight", "att.ln_x.weight", "att.ln_x.bias", "ffn.x_k",
            "ffn.key.weight", "ffn.value.weight"
        ], [
            "emb_cmix_tmix_x", "v_first", "x_tmix_next", "wkv_next",
            "x_cmix_next"
        ], [ln0, ln1, tmix0, tmix_x, ln2, cmix, cmix_x], __opset_imports),
        onnx.helper.make_function(__domain, "block", [
            "emb", "v_first", "x_tmix_last", "wkv_state", "x_cmix_last",
            "ln1.weight", "ln1.bias", "ln2.weight", "ln2.bias", "att.x_r",
            "att.x_w", "att.x_k", "att.x_v", "att.x_a", "att.x_g", "att.w1",
            "att.w2", "att.w0", "att.a1", "att.a2", "att.a0", "att.v1",
            "att.v2", "att.v0", "att.g1", "att.g2", "att.k_k", "att.k_a",
            "att.r_k", "att.receptance.weight", "att.key.weight",
            "att.value.weight", "att.output.weight", "att.ln_x.weight",
            "att.ln_x.bias", "ffn.x_k", "ffn.key.weight", "ffn.value.weight"
        ], ["emb_cmix_tmix_x", "x_tmix_next", "wkv_next", "x_cmix_next"],
                                  [ln0_, ln1, tmix, tmix_x, ln2, cmix, cmix_x],
                                  __opset_imports),
        onnx.helper.make_function(__domain, "block0_a", [
            "emb", "x_tmix_last", "wkv_state", "x_cmix_last", "ln0.weight",
            "ln0.bias", "ln1.weight", "ln1.bias", "ln2.weight", "ln2.bias",
            "att.x_r", "att.x_w", "att.x_k", "att.x_v", "att.x_a", "att.x_g",
            "att.w1", "att.w2", "att.w0", "att.a1", "att.a2", "att.a0",
            ...
            "att.receptance.weight", "att.key.weight", "att.value.weight",
            "att.output.weight", "att.ln_x.weight", "att.ln_x.bias", "ffn.x_k",
            "ffn.key.weight", "ffn.value.weight", "ffn.s1", "semb", "ffn.s2",
            "ffn.s0"
        ], [
            "emb_cmix_tmix_x", "v_first", "x_tmix_next", "wkv_next",
            "x_cmix_next"
        ], [ln0, ln1, tmix0, tmix_x, ln2, cmix_a, cmix_x], __opset_imports),
        onnx.helper.make_function(__domain, "block_a", [
            "emb", "v_first", "x_tmix_last", "wkv_state", "x_cmix_last",
            "ln1.weight", "ln1.bias", "ln2.weight", "ln2.bias", "att.x_r",
            "att.x_w", "att.x_k", "att.x_v", "att.x_a", "att.x_g", "att.w1",
            "att.w2", "att.w0", "att.a1", "att.a2", "att.a0", "att.v1",
            ...
            ], [ln0_, ln1, tmix, tmix_x, ln2, cmix_a, cmix_x], __opset_imports)
    ]
...
def make_sampling(
    ...
    topk: list[onnx.NodeProto] = [
        ...
        onnx.helper.make_node("ReduceSum", ["x_softmax_topk_", "[-1]"],
                              ["x_softmax_topk_sum"]),
        onnx.helper.make_node("Div", ["x_softmax_topk_", "x_softmax_topk_sum"],
                              ["topk_scaled"])
    ]
    ...
    topp: list[onnx.NodeProto] = [
        ...
        onnx.helper.make_node("Div",
                              ["topp_topk_scaled", "topp_topk_scaled_sum"],
                              ["topp_scaled"]),
    ]

    temperature: list[onnx.NodeProto] = [
        onnx.helper.make_node("Reciprocal", ["temp"], ["1/temp"]),
        onnx.helper.make_node("Pow", ["topp_scaled", "1/temp"],
                              ["topp_temp_"]),
        onnx.helper.make_node("ReduceSum", ["topp_temp_", "[-1]"],
                              ["topp_temp_sum"]),
        onnx.helper.make_node("Div", ["topp_temp_", "topp_temp_sum"],
                              ["topp_temp"]),
    ]

    sampling: list[onnx.NodeProto] = [
        onnx.helper.make_node("Cast", ["topp_temp"], ["topp_temp_casted"],
                              to=onnx.TensorProto.FLOAT),
        onnx.helper.make_node("Reshape", ["topp_temp_casted", "B*TK"],
                              ["topp_temp_casted_bt"]),
        onnx.helper.make_node("Log", ["topp_temp_casted_bt"],
                              ["topp_temp_casted_bt_log"]),
        onnx.helper.make_node("Multinomial", ["topp_temp_casted_bt_log"],
                              ["sampled_idx_bt_"],
...
def make_model_from_state_dict(
    ...
    if args.partial is None:
        ...
        nlayers += 1
    unused_key_prefix: list[str] = [
        f"blocks.{layer}" for layer in range(nlayers, nlayers_actual)
    ]
    unused_key: list[str] = [
        key for key in state_dict.keys()
        if any([key.startswith(prefix) for prefix in unused_key_prefix])
    ]
    for k in unused_key:
        state_dict.pop(k)
    ...
    _ = [
        onnx.checker.check_function(loramlp_function)
        for loramlp_function in loramlp_functions
    ]
    onnx.checker.check_function(wkv7_function)
    _ = [
        onnx.checker.check_function(time_mix_function)
    ...
    _ = [
        onnx.checker.check_function(block_function)
    ...
    for k in list(state_dict.keys()):
        if not k.startswith("blocks.0.") and (k.endswith("ln0.weight")
                                              or k.endswith("ln0.bias")):
    ...
    for i in range(nlayers):
        ...
        next_value_infos.append(
            onnx.helper.make_tensor_value_info(
                ...
                ["batch", n_head, head_size, head_size]))
        next_value_infos.append(
            onnx.helper.make_tensor_value_info(f"x_cmix_next_{i}",
                                               onnx_main_dtype,
                                               ["batch", 1, dim]))

    emb: onnx.NodeProto = onnx.helper.make_node("Gather", ["emb.weight", "x"],
                                                ["emb"])
    ...
    for i in range(nlayers):
        ...
        semb.append(
            onnx.helper.make_node("Transpose",
                                  [f"blocks.{i}.ffn.s_emb_x.weight"],
        ...
        semb.append(
            onnx.helper.make_node(
                "Add",
                [f"blocks.{i}.ffn.s_emb.weight", f"blocks.{i}.ffn.s_emb_bias"],
                [f"blocks.{i}.ffn.s_emb.real_weight"]))
        semb.append(
            onnx.helper.make_node("Gather",
                                  [f"blocks.{i}.ffn.s_emb.real_weight", "x"],
                                  [f"blocks.{i}.ffn.semb"]))

    if is_deepemb:
        block0: onnx.NodeProto = onnx.helper.make_node("block0_a", [
            "emb", "x_tmix_last_0", "wkv_state_0", "x_cmix_last_0",
            "blocks.0.ln0.weight", "blocks.0.ln0.bias", "blocks.0.ln1.weight",
            "blocks.0.ln1.bias", "blocks.0.ln2.weight", "blocks.0.ln2.bias",
            ...
            "blocks.0.ffn.x_k", "blocks.0.ffn.key.weight",
            "blocks.0.ffn.value.weight", "blocks.0.ffn.s1",
            "blocks.0.ffn.semb", "blocks.0.ffn.s2", "blocks.0.ffn.s0"
        ], ["emb0", "v_first", "x_tmix_next_0", "wkv_next_0", "x_cmix_next_0"],
                                                       domain=__domain)
    else:
        block0: onnx.NodeProto = onnx.helper.make_node("block0", [
            "emb", "x_tmix_last_0", "wkv_state_0", "x_cmix_last_0",
            "blocks.0.ln0.weight", "blocks.0.ln0.bias", "blocks.0.ln1.weight",
            "blocks.0.ln1.bias", "blocks.0.ln2.weight", "blocks.0.ln2.bias",
            "blocks.0.att.x_r", "blocks.0.att.x_w", "blocks.0.att.x_k",
    ...
    for i in range(1, nlayers):
        if is_deepemb:
            blocks.append(
                onnx.helper.make_node("block_a", [
                    f"emb{i - 1}", "v_first", f"x_tmix_last_{i}",
                    f"wkv_state_{i}", f"x_cmix_last_{i}",
                    ...
                    f"blocks.{i}.att.w1", f"blocks.{i}.att.w2",
                    f"blocks.{i}.att.w0", f"blocks.{i}.att.a1",
                    f"blocks.{i}.att.a2", f"blocks.{i}.att.a0",
                    f"blocks.{i}.att.v1", f"blocks.{i}.att.v2",
                    f"blocks.{i}.att.v0", f"blocks.{i}.att.g1",
                    f"blocks.{i}.att.g2", f"blocks.{i}.att.k_k",
                    f"blocks.{i}.att.k_a", f"blocks.{i}.att.r_k",
                    f"blocks.{i}.att.receptance.weight",
                    f"blocks.{i}.att.key.weight",
                    f"blocks.{i}.att.value.weight",
                    f"blocks.{i}.att.output.weight",
                    f"blocks.{i}.att.ln_x.weight", f"blocks.{i}.att.ln_x.bias",
                    f"blocks.{i}.ffn.x_k", f"blocks.{i}.ffn.key.weight",
                    f"blocks.{i}.ffn.value.weight", f"blocks.{i}.ffn.s1",
                    f"blocks.{i}.ffn.semb", f"blocks.{i}.ffn.s2",
                    f"blocks.{i}.ffn.s0"
                ], [
